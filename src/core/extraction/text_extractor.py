...
Primary extraction method for digital (non-scanned) PDFs.
Uses regex patterns to extract invoice data from extracted text.
"""
...
class TextExtractor:
    """Extract invoice data from digital PDFs using text extraction."""
    ...
    def _clean_text(self, text: str) -> str:
        ...
        # Pattern: letter followed by space then "l" or "i" that should be ligature
        import re
        ...
        # "Freiberu icher" -> check for common German words with fl
        common_fl_words = [
    ...
    def extract(self, pdf_path: str | Path, sender_vat_id: str | None = None) -> ExtractionResult:
        ...
        try:
            ...
            # Clean text (handle ligatures, null chars, etc.)
            full_text = self._clean_text(full_text)

            # Check if PDF appears to be scanned
            if is_likely_scanned_pdf(full_text, page_count):
                warnings.append("PDF appears to be scanned/image-based. OCR may provide better results.")

            # Extract invoice data
            data = self._extract_from_text(full_text, page_count, sender_vat_id=sender_vat_id)
            data.raw_text = full_text
            data.extraction_method = ExtractionMethod.TEXT

            # Calculate overall confidence
            data.overall_confidence = data.calculate_overall_confidence()
    ...
    def _extract_from_text(self, text: str, page_count: int, sender_vat_id: str | None = None) -> ExtractedInvoiceData:
        ...
        if value:
            data.invoice_number = ExtractedField(value=value, confidence=conf, source=pattern)
        ...
        if value:
            ...
            data.invoice_date = ExtractedDateField(
                value=parsed_date,
                confidence=conf if parsed_date else 0.0,
                source=pattern,
                raw_value=value,
            )
        ...
        if value:
            ...
            data.amount_gross = ExtractedDecimalField(
                value=parsed_amount,
                confidence=conf if parsed_amount else 0.0,
                source=pattern,
                raw_value=value,
            )
        ...
        if vat_rate:
            data.vat_rate = ExtractedField(value=vat_rate, confidence=vat_conf, source="vat_rate_detection")
    ...
    def _extract_client_info(self, text: str, sender_vat_id: str | None = None) -> ExtractedClientInfo:
        """Extract client/recipient information from text.
        ...
        if company_name:
            client.name = ExtractedField(
                ...
                confidence=company_name["confidence"],
        ...
        if client_block:
            ...
            if address.get("street"):
                client.street = ExtractedField(value=address["street"], confidence=0.85, source="client_block")
            if address.get("zip_code"):
                client.zip_code = ExtractedField(value=address["zip_code"], confidence=0.9, source="client_block")
            if address.get("city"):
                client.city = ExtractedField(value=address["city"], confidence=0.85, source="client_block")
            if address.get("country"):
                client.country = ExtractedField(value=address["country"], confidence=0.9, source="client_block")

            # Extract email and phone from client block
            email = self._extract_email(client_block)
            if email:
                client.email = ExtractedField(value=email, confidence=0.9, source="client_block")

            phone = self._extract_phone(client_block)
            if phone:
                client.phone = ExtractedField(value=phone, confidence=0.85, source="client_block")

        # Step 5: If no country yet, infer from VAT prefix
        if not client.country.value and client_vat_country:
            client.country = ExtractedField(value=client_vat_country, confidence=0.8, source="vat_prefix")
        ...
        for search_text, conf_multiplier in search_texts:
            for pattern, base_confidence in company_patterns:
                ...
                if match:
                    ...
                    sender_keywords = ["freiberuflich", "freelance", "engineer", "consultant", "berater", "entwickler"]
    ...
    def _extract_payment_terms(self, text: str) -> ExtractedField:
        ...
        for pattern, confidence in payment_patterns:
            ...
            if match:
                ...
                return ExtractedField(value=terms, confidence=confidence, source=pattern)
    ...
    def _extract_line_items(self, text: str) -> tuple[list, ExtractedField]:
        ...
        if descriptions:
            ...
            description_field = ExtractedField(
                value=desc_text, confidence=0.9, source="line_items_table"
            )
        ...
        for line in lines:
            ...
            if len(line) > 30 and not any(
                ...
                return [], ExtractedField(value=line, confidence=0.4, source="fallback")
